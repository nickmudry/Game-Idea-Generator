...
//TODO: Make this a random picker from the DB. 
	...
	if ($_GET['share'] == "true")
	{
		$comboName = ($game1Name."-".$game2Name);
		//Fetch Combinations Database Where Name = 'game1-game2'
		$comboRequest = mysql_query("SELECT * FROM combinations where name='$comboName'");
		echo ($comboRequest);
		
		if (mysql_fetch_row($comboRequest) == TRUE)
		{
			echo ("found combo in DB");
			$comboFetch = mysql_fetch_array($comboRequest);
			$votesUpdate = $comboFetch['votes'] + 1;
			$comboUpdate = mysql_query("UPDATE combinations SET votes='$votesUpdate' WHERE combination='$comboName'");
			echo ("updated $comboName votecount to $votesUpdate");
		}
		else
		{	
			echo ("Doesnt Exist Adding");
			$comboAdd = mysql_query("INSERT INTO combinations (combination, votes) VALUES ('$comboName',1)");
			echo ("Added $comboName to DB. Check to confirm.");
		}
		//if doesnt exist, then create it 
		
		//else if exists, add a vote
